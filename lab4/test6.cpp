...
int main() {
  ...
  int numBytes = sendto(sock, s.c_str(), s.length(), 0, (struct sockaddr *)&sin,
                        sizeof(sin));
  ...
  numBytes =
      recvfrom(sock, buf2, sizeof(buf2), 0, (struct sockaddr *)&sin, &sin_size);
  cout << "Recevied: " << numBytes << endl;
  cout << "From " << inet_ntoa(sin.sin_addr) << endl;

  Person *p2 = new Person;
  p2->ParseFromString(std::string(buf2, numBytes));
